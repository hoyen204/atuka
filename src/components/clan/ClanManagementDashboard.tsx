...
interface ShopItem {
  ...
  category: "tong-mon" | "phap-bao" | "dan-duoc";
  actionType?: "buy" | "sell";
  bonus?: string;
  usageLimit?: number;
  usedCount?: number;
...
export default function ClanManagementDashboard() {
  ...
  useEffect(() => {
    async function fetchClanDetail() {
      if (!selectedAccountId) {
        router.push("/dashboard/clan-selection");
        return;
      }
          ...
          for (let i = 0; i < quantityToBuy; i++) {
            try {
              ...
              if (buyRes.ok && buyData.success) {
                ...
                totalErrors++;
                toast.error(
                  `[${account.name}] Lỗi khi mua ${item.name}: ${
                    buyData.message || "Lỗi không rõ"
                  }`
                );
              }
            } catch (e: any) {
  ...
  return (
    <TooltipProvider>
      <div className="container mx-auto p-6 space-y-6">
        ...
        <div className="flex justify-between items-center">
          <div className="text-center flex-grow">
            <h1 className="text-4xl font-bold text-gray-800 mb-2 flex items-center justify-center gap-3">
              ...
              {clanDetail.header?.name || "Loading..."} - Cấp{" "}
              {clanDetail.header?.level || "..."}
            </h1>
              ...
              <SelectContent>
                ...
                {proxies.map((p) => (
                  <SelectItem
                    key={p.id}
                    value={p.id}
                  >{`${p.username}:${p.password}@${p.host}:${p.port}`}</SelectItem>
                ))}
              </SelectContent>
            </Select>
          </div>
        ...
        {/* Stats Overview using clanDetail.header */}
              ...
              <CardContent>
                ...
                >
                  <TabsList className="grid w-full grid-cols-3">
                    ...
                    <TabsTrigger value="phap-bao">Pháp Báo</TabsTrigger>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                      {clanDetail.shopData.tongMonItems.map((item: any) => (
                        ...
                        >
                          ...
                          <CardContent className="p-0 pt-2 flex flex-col gap-2 flex-grow">
